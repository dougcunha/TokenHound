...
3. In the executor, read the stable report and task once per version; then retrieve relevant PRD/TechSpec sections, code, and skills. Trace the cause and implement within the limits. Apply TechSpec validation; in desktop C#/.NET, omit E2E even in legacy commands. Record unit, integration, required manual checks, and limitations; when available, use `dotnet-efficient-validation`.
